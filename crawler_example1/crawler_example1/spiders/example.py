...
class ExampleSpider(scrapy.Spider):
    ...
    def parsing_pages(self, response):
        yield Request(url=response.url,
                      ...
                      dont_filter=True)
        next_page_url = response.xpath('//*[@class="pagination"]')
        for a in next_page_url.xpath('a'):
            is_actived = a.xpath('//*[@class="actived"]')
            if is_actived is None:
                number = a.xpath('//*[@class="actived"]/a/text()').extract()


    def page_info(self, response):
        body = response.xpath('//*[@class="description')
        a = body
        # for info in body.xpath('div'):
        #     url = info.xpath('a/@href').extract_first()
        #     yield Request(url='http://www.phimmoizz.net/' + url, callback=self.extract_movie, dont_filter=True)
